<template>
  ...
  >
    ...
    <t-input-group class="groups">
      ...
      <t-input type="text" class="ranges" placeholder="请输入你要录入的职位等级"/>
    </t-input-group>

    <t-input-group class="groups">
      <span class="titles">职位薪资：</span>
      <t-input type="text" class="ranges" placeholder="请输入你要录入的职位薪资"/>
    </t-input-group>
  </t-dialog>
  <t-col :span="8">
    <t-drawer v-model:visible="visible" size="medium" :header="header" :on-confirm="drawerClose" :close-btn="true">
      <t-col :span="12">
        <t-input-group class="groups">
          <span class="titles">职位：</span>
          <t-input type="text" class="ranges" placeholder="请输入你要修改的职位"/>
        </t-input-group>

        <t-input-group class="groups">
          <span class="titles">职位等级：</span>
          <t-input type="text" class="ranges" placeholder="请输入你要修改的职位等级"/>
        </t-input-group>

        <t-input-group class="groups">
          <span class="titles">职位薪资：</span>
          <t-input type="text" class="ranges" placeholder="请输入你要修改的职位薪资"/>
        </t-input-group>
      </t-col>
    </t-drawer>
  </t-col>
      ...
      <t-row>
        <t-col :span="10">
          <t-input-group class="group">
            ...
            <t-input type="text" class="range" v-model="position.dname" placeholder="请输入您要查询的职位"/>
          </t-input-group>

          <t-input-group class="group">
            <span class="title">职位等级：</span>
            <t-input type="text" class="range" v-model="position.position" placeholder="请输入您要查询的职位等级"/>
          </t-input-group>

          <t-input-group class="group">
            <span class="title">职位薪资：</span>
            <t-input type="text" class="range" v-model="position.duty" placeholder="请输入您要查询的职位薪资"/>
          </t-input-group>
        </t-col>
...
const columns = ref([
  { colKey: 'pid', title: '职位序号'},
  { colKey: 'plevel', title: '职位等级'},
  { colKey: 'psalary', title: '职位薪资'},
